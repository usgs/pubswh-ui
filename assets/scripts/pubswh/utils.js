 ...
 * @param {Array}  arrayWithDuplicates - an array of objects that have properties with duplicate values
...
export function removeObjectsWithDuplicateValues(arrayWithDuplicates, propertyName) {
    ...
    for (let i = 0; i < arrayWithDuplicates.length; i++) {
        ...
        let indexOfDuplicate = deduplicatedArray.findIndex(objectFromArray => objectFromArray.text === arrayWithDuplicates[i][propertyName]);
        // if the value is not in the new list that has no duplicates add the object containing that value to the list,
        // otherwise do nothing
            if (indexOfDuplicate === -1) {
                            deduplicatedArray.push(arrayWithDuplicates[i]);
            }
    }
