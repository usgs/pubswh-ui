...
from pubs_ui.utils import pull_feed, pubdetails, getbrowsecontent, create_display_links, jsonify_geojson, add_supersede_data,SearchPublications, make_contributor_list, preceding_and_superseding
...
def use_preceding_superceding(step):
    world.preceding_superseding_dict = preceding_and_superseding('fs03700','http://pubs-fake.er.usgs.gov/service/citation/json/extras')
    world.expected_preceding_superseding_dict = {'successors': [{'date': u'2001', 'index_id': u'fs03301', 'title': u'U.S. Geological Survey World Wide Web Information'}, {'date': u'2003', 'index_id': u'fs05503', 'title': u'U.S. Geological Survey World Wide Web Information'}], 'context_item': 'fs03700', 'predecessors': [{'date': u'1999', 'index_id': u'fs07199', 'title': u'U.S. Geological Survey World Wide Web Information'}]}
...
@step(r'I pass those variables to add_supersede_data')
def add_supersede_pubs(step):
    world.fs03700_pubdata_with_supersede = add_supersede_data(world.fs03700_pubdata,
